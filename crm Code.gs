// Deep Pocket CRM — Apps Script backend v2.8 (idempotent / duplicate-proof)
...
const SHEET_ID='1SxawbtS7s5EiDqGdDlOT5JqJPsuFVzUwX3Bp1qLvdI4';
// Central, permanent feedback sheet — owned by Deep Pocket (the vendor),
// collects client feedback from EVERY firm's deployment of this CRM into
// one place, regardless of which Google account each firm's own script
// runs under. Deliberately a different spreadsheet than SHEET_ID above —
...
function callGemini(opts){
  ...
  // Opt-in structured-JSON mode — asks Gemini itself to constrain output to
  // valid JSON, which is meaningfully more reliable than just instructing
  // it in the prompt text and hoping. Used by the Practice Intelligence
  // Centre and daily insights email below, both of which parse the result
  // as JSON and need to handle a malformed response gracefully either way.
  if(opts.jsonMode){ payload.generationConfig.responseMimeType = 'application/json'; }
...
function callGeminiAPI(summaryText){
  var prompt='You are a productivity analyst for Deep Pocket Fintech, an Indian wealth-management advisory firm. '
...
function draftLeadMessage(data){
  ...
  var prompt='You are drafting a short WhatsApp message from a wealth management advisor at Deep Pocket Fintech '
...
function generateLeadProfileInsight(data){
  ...
  var prompt='You are helping a wealth management advisor at Deep Pocket Fintech understand a LEAD (not yet converted) '
...
function generateClientProfileInsight(data){
  ...
  var prompt='You are helping a wealth management advisor at Deep Pocket Fintech understand an EXISTING CLIENT '
    +'in depth, using everything tracked about them below. This is an internal profile for the advisor\'s own understanding — '
...
function generateMeetingInsight(data){
  ...
  var promptText=panIsolationNote
    +'You are preparing a wealth management advisor at Deep Pocket Fintech for an upcoming in-person or review '
...
// ═══════════════════════════════════════════════
// PRACTICE INTELLIGENCE CENTRE — evolves Insights from a generic chatbot
// into structured, evidence-based analysis across 14 fixed questions in 3
// categories. Deliberately separate from practiceChat (free-form Q&A)
// above and generateContemporaryInsight (web-search briefing) — this is
// the third, most structured tier: fixed questions, fixed JSON shape,
// rendered as a proper report rather than a chat reply. Reuses
// gatherPracticeContext() for data (same proven, compact data-gathering
// already used by practiceChat) rather than re-deriving CRM data a
// second, parallel way.
// ═══════════════════════════════════════════════

var INSIGHT_QUESTIONS = {
  // Lead Intelligence
  lead_at_risk: {category:'lead', label:'Which leads are most at risk of being lost?',
    instruction:'Analyse lead age, stage, source, activity history, communication frequency, last activity, pending and overdue tasks. Rank at-risk leads and recommend action.'},
  lead_best_opportunities: {category:'lead', label:'Which leads represent my best conversion opportunities?',
    instruction:'Analyse engagement, stage, source, requirements and activity patterns. Identify promising leads and recommend the next action.'},
  lead_stuck: {category:'lead', label:'Which leads have been stuck for too long?',
    instruction:'Identify unusually stagnant leads, analyse the available reasons and recommend whether to revive, progress or close them.'},
  lead_followup_breakdown: {category:'lead', label:'Where is my follow-up process breaking down?',
    instruction:'Identify delayed follow-ups, neglected leads, overdue tasks and broader workflow patterns.'},
  lead_next_actions: {category:'lead', label:'What should I do next to improve my active leads?',
    instruction:'Provide a prioritised, data-driven next-action list.'},
  // Client Intelligence
  client_needs_attention: {category:'client', label:'Which client relationships need attention?',
    instruction:'Analyse communication gaps, unresolved issues, pending tasks, feedback and review history.'},
  client_service_opportunities: {category:'client', label:'What client service opportunities am I missing?',
    instruction:'Identify evidence-based opportunities for legitimate advisor discussions. Do not invent client needs or provide unsupported regulated advice.'},
  client_review_priority: {category:'client', label:'Which clients should I review first?',
    instruction:'Prioritise clients using last review date, communication, pending work, feedback and available client information.'},
  client_health: {category:'client', label:'How healthy are my client relationships overall?',
    instruction:'Analyse engagement, service quality, communication gaps and feedback patterns.'},
  // Practice Intelligence
  practice_health_check: {category:'practice', label:'Give me a complete health check of my practice',
    instruction:'Identify top strengths, risks, opportunities and three immediate actions.'},
  practice_efficiency: {category:'practice', label:'Where is my team losing time or efficiency?',
    instruction:'Identify bottlenecks, repeated work, overdue tasks and automation opportunities.'},
  practice_growth_limits: {category:'practice', label:'What is limiting the growth of my practice?',
    instruction:'Analyse the lead funnel and identify possible bottlenecks in lead quality, follow-up, progression and conversion.'},
  practice_automate: {category:'practice', label:'What should I automate or simplify?',
    instruction:'Identify recurring workflows suitable for reminders, templates, AI or automation. Rank by impact and ease.'},
  practice_urgent_ops: {category:'practice', label:'What operational issues require immediate attention?',
    instruction:'Scan for overdue tasks, stagnant leads, forgotten follow-ups, incomplete records and unresolved matters.'}
};

// Compact, pre-aggregated CRM data for AI analysis — never the raw sheets
// themselves. A thin, purposefully-named wrapper around the existing,
// already-proven gatherPracticeContext() rather than a second parallel
// data-gathering implementation that could drift out of sync with it.
function getCRMDataForAnalysis(){
  var ctx=gatherPracticeContext();
  return ctx.stable+'\n\n'+ctx.volatile;
}
...
function callGeminiForInsights(instructionText, crmDataText){
  ...
  var systemPrompt='You are an AI Practice Intelligence analyst for a BFSI wealth-management/insurance advisory firm using '
    ...
    +'carefully and answer the specific question asked. Be rigorous and evidence-based: every finding must be traceable to '
    +'something actually present in the data below. NEVER invent a name, number, date, or interaction that is not in the '
    +'data. If the data is insufficient to answer confidently, say so plainly in the summary rather than guessing. Clearly '
    +'separate FACT from INFERENCE from RECOMMENDATION: "observation" states what the data shows, "evidence" cites the '
    +'specific data point(s) behind it, "recommendedAction" is your suggested next step — never blur these together, and '
    ...
    +'Rules: "priority" reflects overall urgency. "findings" should have 3-8 items, each about one specific lead/client/'
    +'task/pattern — use [] only if truly nothing relevant was found, never invent filler to reach a count. "pattern" is '
    +'one sentence naming the single most important cross-cutting theme, or "" if none. "recommendedActions" is a short '
    +'prioritised list (2-5 items), most important first.\n\nPRACTICE DATA:\n'+crmDataText;

  return callGemini({prompt:systemPrompt, maxOutputTokens:2000, jsonMode:true});
}

// Never trusts the AI's JSON blindly — malformed output or a field of the
// wrong type/shape is normalized to a safe default rather than breaking
// the UI or silently passing through garbage.
function validateAIResponse(raw){
  var fallback={headline:'Could not generate insight', summary:'The AI response could not be parsed — please try again.', priority:'LOW', findings:[], pattern:'', recommendedActions:[]};
  ...
  try{
    ...
    fallback.summary='The AI response was not valid JSON: '+e.message;
    return fallback;
  }
  var validPriority=function(p){ p=String(p||'').toUpperCase(); return ['HIGH','MEDIUM','LOW'].indexOf(p)>=0 ? p : 'MEDIUM'; };
  return {
    headline: String(parsed.headline||'Insight'),
    summary: String(parsed.summary||''),
    priority: validPriority(parsed.priority),
    findings: Array.isArray(parsed.findings) ? parsed.findings.map(function(f){
      f=f||{};
      return {
        entityType: String(f.entityType||'general'),
        entityName: String(f.entityName||''),
        priority: validPriority(f.priority),
    ...
    recommendedActions: Array.isArray(parsed.recommendedActions) ? parsed.recommendedActions.map(function(a){return String(a);}) : []
  };
}
...
function practiceChat(data){
  ...
  var systemPrompt='You are a sharp, warm, genuinely knowledgeable practice-management assistant for a wealth management '
    +'advisor at Deep Pocket Fintech. You have full visibility into their real leads, clients, tasks, AND their actual '
...
function generateClientReviewReport(data){
  ...
  var promptText='Write a warm, personal investment review letter addressed DIRECTLY to a wealth management client, from their advisor '
    +'Arjun K T M at Deep Pocket Fintech Pvt. Ltd.. This will be read BY THE CLIENT THEMSELVES — not an internal document. Tone: warm, '
...
// ═══════════════════════════════════════════════
// DAILY AI INSIGHTS EMAIL — evolved from a hardcoded-recipient, fixed
// Monday-only, plain-text version into a Settings-driven daily briefing.
// Split into the 3 pieces the spec asks for (generate / render / send)
// rather than one function doing everything, so each is independently
// testable and reusable — e.g. the test-send action reuses the exact same
// generate+render path as the real scheduled send, just skipping the
// enabled/dedup checks.
// ═══════════════════════════════════════════════

// Produces the 4 email sections as structured data — never raw prose —
// so createDailyEmailHTML can render it consistently and a malformed AI
// response degrades to empty sections rather than a broken email.
function generateDailyFirmInsights(){
  var crmData=getCRMDataForAnalysis();
  var schema='{"requiresAttention":[{"title":"","detail":""}],"opportunities":[{"title":"","detail":""}],"importantObservation":"","todaysPriorities":[""]}';
  var systemPrompt='You are the daily AI practice manager for a BFSI wealth-management/insurance advisory firm using this '
    +'CRM. Review the firm\'s actual CRM data below (leads, clients, tasks, activity log) and produce today\'s briefing. Be '
    +'evidence-based — every item must be traceable to the actual data below, never invented. If there is genuinely '
    +'nothing notable for a section, return an empty array/string for it rather than manufacturing filler just to fill '
    +'the section.\n\n'
    +'Respond with ONLY valid JSON matching exactly this shape:\n'+schema+'\n\n'
    +'"requiresAttention": urgent risks — leads about to be lost, clients going quiet, overdue tasks, compliance-relevant '
    +'gaps (2-5 items). "opportunities": genuine, evidence-based opportunities — hot leads, upsell/cross-sell signals, '
    +'birthdays/anniversaries worth acting on (2-5 items). "importantObservation": ONE sentence naming the single most '
    +'important pattern across the whole practice today. "todaysPriorities": a short, concrete, prioritised to-do list '
    +'for today specifically (3-6 items).\n\nPRACTICE DATA:\n'+crmData;

  var raw=callGemini({prompt:systemPrompt, maxOutputTokens:1800, jsonMode:true});
  var fallback={requiresAttention:[],opportunities:[],importantObservation:'',todaysPriorities:[]};
  try{
    var jm=raw.match(/\{[\s\S]*\}/);
    var parsed=JSON.parse(jm?jm[0]:raw);
    return {
      requiresAttention: Array.isArray(parsed.requiresAttention)?parsed.requiresAttention:[],
      opportunities: Array.isArray(parsed.opportunities)?parsed.opportunities:[],
      importantObservation: String(parsed.importantObservation||''),
      todaysPriorities: Array.isArray(parsed.todaysPriorities)?parsed.todaysPriorities:[]
    };
  }catch(e){
    fallback.importantObservation='Could not generate today\'s insights — the AI response could not be parsed.';
    return fallback;
  }
}

// Plain, table-based HTML — the layout email clients actually render
// consistently, unlike modern CSS layout techniques which many strip.
function createDailyEmailHTML(insights, firmName){
  function escH(s){ return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
  function listBlock(items, renderItem){
    if(!items || !items.length) return '<p style="color:#999;font-size:13px;margin:4px 0 20px;font-style:italic">Nothing notable today.</p>';
    return '<ul style="margin:4px 0 20px;padding-left:20px">'+items.map(renderItem).join('')+'</ul>';
  }
  var attentionHtml=listBlock(insights.requiresAttention, function(i){
    return '<li style="margin-bottom:10px;color:#333;font-size:13px;line-height:1.5"><b>'+escH(i.title)+'</b>'+(i.detail?' — '+escH(i.detail):'')+'</li>';
  });
  var oppHtml=listBlock(insights.opportunities, function(i){
    return '<li style="margin-bottom:10px;color:#333;font-size:13px;line-height:1.5"><b>'+escH(i.title)+'</b>'+(i.detail?' — '+escH(i.detail):'')+'</li>';
  });
  var prioritiesHtml=listBlock(insights.todaysPriorities, function(p){
    return '<li style="margin-bottom:8px;color:#333;font-size:13px;line-height:1.5">'+escH(p)+'</li>';
  });
  var todayStr=Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'EEEE, MMMM d, yyyy');

  return '<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f4f4f4;font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif">'
    +'<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f4;padding:24px 0">'
    +'<tr><td align="center">'
    +'<table role="presentation" width="100%" style="max-width:560px;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08)">'
    +'<tr><td style="background:#1B1512;padding:26px 28px">'
    +'<div style="color:#F2E8DE;font-size:20px;font-weight:800">'+escH(firmName)+' AI Insights</div>'
    +'<div style="color:#A99A8F;font-size:13px;margin-top:4px">'+todayStr+'</div>'
    +'</td></tr>'
    +'<tr><td style="padding:26px 28px">'
    +'<div style="font-size:15px;font-weight:800;color:#C1553D;margin-bottom:8px">\uD83D\uDD34 Requires Attention</div>'
    +attentionHtml
    +'<div style="font-size:15px;font-weight:800;color:#4C9A6A;margin-bottom:8px">\uD83D\uDFE2 Opportunities</div>'
    +oppHtml
    +'<div style="font-size:15px;font-weight:800;color:#333;margin-bottom:8px">\uD83D\uDCCA Important Practice Observation</div>'
    +'<p style="color:#333;font-size:13px;line-height:1.5;margin:4px 0 20px">'+(escH(insights.importantObservation)||'—')+'</p>'
    +'<div style="font-size:15px;font-weight:800;color:#333;margin-bottom:8px">\uD83D\uDCCB Today\'s Priorities</div>'
    +prioritiesHtml
    +'</td></tr>'
    +'<tr><td style="padding:18px 28px;border-top:1px solid #eee;text-align:center">'
    +'<div style="color:#999;font-size:11px">Generated automatically by your CRM\'s AI Practice Intelligence Centre.</div>'
    +'</td></tr>'
    +'</table></td></tr></table></body></html>';
}

// Orchestrates the send: enabled check, once-per-day dedup (tracked via
// Settings, not a hardcoded flag), dynamic firm name + recipient (never
// hardcoded), logs every attempt — success or failure — to the INSIGHTS
// tab. force=true (used by the test-send action) bypasses the
// enabled/dedup checks so a test always actually sends, regardless of
// today's automatic-send state.
function sendDailyInsightsEmail(force){
  var enabled=getSettingValue('dailyInsightsEnabled','')==='true';
  if(!enabled && !force) return {success:false, skipped:true, error:'Daily AI Insights is disabled in Settings'};

  var todayKey=Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  if(!force && getSettingValue('dailyInsightsLastSentDate','')===todayKey){
    return {success:true, skipped:true, reason:'Already sent today'};
  }

  var recipient=getSettingValue('dailyInsightsRecipient','');
  if(!recipient) return {success:false, error:'No recipient email configured in Settings'};
  var firmName=getSettingValue('firmName','')||'Your Firm';
  ...
  try{
    var insights=generateDailyFirmInsights();
    var html=createDailyEmailHTML(insights, firmName);
    MailApp.sendEmail({to:recipient, subject:firmName+' AI Insights', htmlBody:html});
    setSettingValue('dailyInsightsLastSentDate', todayKey);
    iSh.appendRow([stamp, JSON.stringify(insights)]);
    return {success:true};
  }catch(e){
    iSh.appendRow([stamp, 'ERROR: '+e.message]);
    console.error('Daily insights email failed:', e.message); // logged, never silently swallowed
    return {success:false, error:e.message};
  }
}

// Kept as its own function and as the exact name already registered as a
// trigger handler by any existing deployment — internally delegates to
// the modular pieces above rather than duplicating their logic, so the
// trigger itself never needs to be recreated just because the internals
// changed.
function generateAndSendInsights(){
  return sendDailyInsightsEmail(false);
}

// ── Trigger management — now DAILY (not weekly) at a Settings-configured
// hour, instead of a hardcoded Monday-8am schedule. One real limitation
// worth being explicit about: Apps Script time-based triggers fire
// relative to the SCRIPT PROJECT's own timezone (Project Settings > Time
// zone in the Apps Script editor) — there is no per-trigger IANA timezone
// parameter. The "timezone" Settings field is used for correctly labeling
// dates in the email itself and for display, but for the trigger's actual
// firing hour to match a firm's real local time, the Apps Script project's
// timezone must itself be set to that firm's timezone. This mirrors the
// spec's own note that exact timing can't be guaranteed — treat the
// configured hour as "approximately", not to-the-minute precise. ──
function setupInsightsTrigger(){
  removeInsightsTrigger(); // avoid duplicates on re-enable
  var hour=parseInt(getSettingValue('dailyInsightsHour','8'),10);
...
// ═══════════════════════════════════════════════
// AUTOMATIC CLIENT EMAIL → TASK
//
// Periodically checks the Gmail inbox connected to this deployment,
// matches the sender against real clients in the existing CLIENTS sheet
// (column C = Email), and creates a task for the client's owning staff
// member via the EXISTING addTask logic — reused through writeData()
// exactly like the server-side diagnostic elsewhere in this file does,
// rather than a second, parallel task-creation path that could drift out
// of sync with the real one.
//
// "Client Owner" is resolved from CLIENTS column 42 ("User ID") looked up
// against the existing Admin sheet (same A=Name/B=Username/C=UserType/
// D=UserID layout verifyIsAdmin already relies on) — no new columns
// added to either sheet. The Tasks sheet has no separate "assigned to"
// field (its old one was long-repurposed into "Created By" — see the
// comment on addTask above), so the resolved owner's name is written into
// "Created By" for this auto-generated task, which is the closest
// existing, real field to "who this belongs to."
//
// Duplicate prevention is two layers, per the spec's explicit ask for a
// robust combination: (1) a Gmail label (CRM_TASK_PROCESSED) excluded
// directly in the search query, so an already-handled thread is never
// even fetched on a later run — efficient, no full-mailbox rescans; (2) a
// lightweight PROCESSED_EMAILS sheet logging every handled message ID, as
// a redundant check in case a label somehow didn't apply. A task is only
// ever labeled AFTER it's successfully created — if creation fails, the
// email is deliberately left unlabeled so the next run retries it rather
// than silently losing it.
// ═══════════════════════════════════════════════

var CLIENT_EMAIL_LABEL_NAME='CRM_TASK_PROCESSED';
var PROCESSED_EMAILS_TAB='PROCESSED_EMAILS';

function getOrCreateProcessedLabel(){
  var label=GmailApp.getUserLabelByName(CLIENT_EMAIL_LABEL_NAME);
  if(!label) label=GmailApp.createLabel(CLIENT_EMAIL_LABEL_NAME);
  return label;
}

// { emailLower: {name, recordId, userId} } — read straight from the live
// CLIENTS sheet using its exact existing column layout (see appendClient
// above: Email is column 3, Record ID is column 35, User ID is column 42).
function buildClientEmailIndex(){
  var ss=SpreadsheetApp.openById(SHEET_ID);
  var cSh=ss.getSheetByName(CLIENTS_TAB);
  var idx={};
  if(!cSh || cSh.getLastRow()<2) return idx;
  var vals=cSh.getRange(2,1,cSh.getLastRow()-1,42).getValues();
  vals.forEach(function(r){
    var email=String(r[2]||'').trim().toLowerCase(); // column C = Email
    if(!email) return;
    idx[email]={ name:String(r[0]||''), recordId:String(r[34]||''), userId:String(r[41]||'') }; // Name, Record ID, User ID
  });
  return idx;
}

// Resolves a client's User ID into the actual owning staff member's name
// + role via the existing Admin sheet — a standalone, globally-callable
// twin of verifyIsAdmin's lookup (that one is nested inside writeData and
// unreachable from here), same exact column layout.
function resolveClientOwner(userId){
  if(!userId) return {name:'', role:''};
  var ss=SpreadsheetApp.openById(SHEET_ID);
  var adSh=ss.getSheetByName(ADMIN_TAB);
  if(!adSh || adSh.getLastRow()<2) return {name:'', role:''};
  var vals=adSh.getRange(2,1,adSh.getLastRow()-1,4).getValues(); // A-D: Name, Username, UserType, UserID
  for(var i=0;i<vals.length;i++){
    if(String(vals[i][3]||'').trim()===String(userId).trim()){
      return {name:String(vals[i][0]||''), role:String(vals[i][2]||'').trim().toUpperCase()};
    }
  }
  return {name:'', role:''};
}
...
function isEmailAlreadyProcessed(messageId){
  var ss=SpreadsheetApp.openById(SHEET_ID);
  var sh=ss.getSheetByName(PROCESSED_EMAILS_TAB);
  if(!sh || sh.getLastRow()<2) return false;
  var ids=sh.getRange(2,1,sh.getLastRow()-1,1).getValues();
  for(var i=0;i<ids.length;i++){
    if(String(ids[i][0]||'').trim()===messageId) return true;
  }
  return false;
}
function markEmailProcessed(messageId, clientName, taskId){
  var ss=SpreadsheetApp.openById(SHEET_ID);
  var sh=ss.getSheetByName(PROCESSED_EMAILS_TAB)||ss.insertSheet(PROCESSED_EMAILS_TAB);
  if(sh.getLastRow()===0){
    var hdr=[['Message ID','Processed Date','Client Name','Task ID']];
    var hr=sh.getRange(1,1,1,4);
    hr.setValues(hdr);
    hr.setFontWeight('bold').setBackground('#C0392B').setFontColor('#FFFFFF');
    sh.setFrozenRows(1);
  }
  sh.appendRow([messageId, Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm:ss'), clientName, taskId]);
}

// Creates the task by calling the EXISTING addTask action through
// writeData() — the exact same code path a human tapping "Save" on the
// Add Task form goes through, so Task ID generation, sheet self-healing,
// and column layout can never drift between the two.
//
// Schema mapping (no new columns anywhere): Name = the client's name
// (matches what this field already means everywhere else in the app —
// who the task is about, not a task title); Task Details = the email
// subject + body + received time + thread ID folded into one field,
// since there's no separate "title" column to hold the subject alone;
// Created By = the resolved owner's name (the closest existing field to
// "assigned to," since the Tasks sheet's real "Assigned To" column was
// long ago repurposed — see the note on addTask above).
function createTaskFromClientEmail(client, subject, body, receivedDate, threadId){
  var today=Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  var owner=resolveClientOwner(client.userId);
  var receivedStr=Utilities.formatDate(receivedDate, Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm');
  var bodyTrimmed=String(body||'').trim().slice(0,2000); // capped — a task detail field, not a full email archive
  var details='Email Request: '+subject
    +'\n\n'+bodyTrimmed
    +'\n\n— Received '+receivedStr+' | Gmail thread: '+threadId;
  ...
  var payload={
    ...
    status: 'Open',
    taskFor: 'CLIENT',
    linkedRecordId: client.recordId,
    createdBy: owner.name || client.userId || 'Unassigned',
    createdByRole: owner.role || ''
  };
  return JSON.parse(writeData(payload).getContent());
}

// ── Main entry point — run on a time-driven trigger (see setup function
// below). Searches only unlabeled inbox mail; Gmail's own search index
// already excludes Sent/Spam/Trash from "in:inbox", and the label
// exclusion means an already-handled thread is never re-fetched on a
// later run at all, let alone re-processed. ──
function checkClientEmailsForTasks(){
  var label=getOrCreateProcessedLabel();
  var clientIndex=buildClientEmailIndex();
  var scriptOwnEmail=Session.getEffectiveUser().getEmail();

  // A plain Settings field can't redirect which mailbox GmailApp actually
  // reads (fixed by authorization, not a runtime setting) — but it CAN
  // catch a mismatch loudly, right in the Executions log, instead of this
  // silently running against the wrong inbox with zero visible sign why.
  var configuredInbox=getSettingValue('clientEmailMonitoringInbox','');
  if(configuredInbox && configuredInbox.toLowerCase()!==scriptOwnEmail.toLowerCase()){
    console.warn('checkClientEmailsForTasks: MISMATCH — configured inbox is "'+configuredInbox
      +'" but this script is actually authorized as "'+scriptOwnEmail+'". This run is reading '
      +scriptOwnEmail+', not the configured address. Either re-authorize this script under '
      +configuredInbox+', or set up mail forwarding from '+configuredInbox+' into '+scriptOwnEmail+'.');
  }

  var threads;
  try{
    // Capped per run, not because of a size limit concern but so a
    // backlog works through steadily over a few trigger firings rather
    // than one very long-running execution.
    threads=GmailApp.search('in:inbox -label:'+CLIENT_EMAIL_LABEL_NAME, 0, 50);
  }catch(searchErr){
    console.error('checkClientEmailsForTasks: Gmail search failed:', searchErr.message);
  ...
  threads.forEach(function(thread){
    try{
      ...
      var msg=messages[messages.length-1]; // most recent message in the thread
      var messageId=msg.getId();

      if(isEmailAlreadyProcessed(messageId)){ thread.addLabel(label); skipped++; return; }

      var fromHeader=msg.getFrom(); // e.g. "Jane Doe <jane@example.com>"
      var fromMatch=fromHeader.match(/<([^>]+)>/);
      var fromEmail=(fromMatch?fromMatch[1]:fromHeader).trim().toLowerCase();

      // The two filters the whole feature depends on: never act on the
      // firm's own mailbox, and only ever act on a recognised client.
      if(fromEmail===scriptOwnEmail.toLowerCase()){ thread.addLabel(label); skipped++; return; }
      var client=clientIndex[fromEmail];
      if(!client){ skipped++; return; } // unrecognised sender — left untouched, not this feature's mail to manage

      var subject=msg.getSubject()||'(no subject)';
      var body=msg.getPlainBody()||'';
      var receivedDate=msg.getDate();

      var taskResult=createTaskFromClientEmail(client, subject, body, receivedDate, thread.getId());
      if(taskResult && taskResult.success){
        markEmailProcessed(messageId, client.name, taskResult.taskId);
        thread.addLabel(label);
        created++;
      } else {
        // Deliberately NOT labeled on failure — the next run retries this
        // email instead of it being silently lost forever.
        console.error('checkClientEmailsForTasks: task creation failed for', fromEmail, taskResult&&taskResult.error);
        errors++;
      }
    }catch(threadErr){
      console.error('checkClientEmailsForTasks: error processing a thread:', threadErr.message);
      errors++;
    }
  });

  console.log('checkClientEmailsForTasks: created='+created+' skipped='+skipped+' errors='+errors);
  return {success:true, created:created, skipped:skipped, errors:errors};
}

// ── Trigger management — same pattern as setupInsightsTrigger/
// removeInsightsTrigger above: always remove first, so re-running setup
// can never create duplicate triggers. ──
function setupClientEmailMonitoringTrigger(){
  removeClientEmailMonitoringTrigger();
  ScriptApp.newTrigger('checkClientEmailsForTasks')
    .timeBased().everyMinutes(15).create();
  return {success:true};
}
function removeClientEmailMonitoringTrigger(){
  var triggers=ScriptApp.getProjectTriggers();
  var removed=0;
  triggers.forEach(function(t){
    if(t.getHandlerFunction()==='checkClientEmailsForTasks'){ ScriptApp.deleteTrigger(t); removed++; }
  });
  return {success:true, removed:removed};
}

// ── Configured-inbox validation — a plain Settings text field cannot
// actually redirect which mailbox GmailApp reads (that's fixed by
// whichever account authorized the script, not changeable at runtime).
// What this CAN do, honestly: let the admin record which inbox they
// BELIEVE is being monitored, and flag it loudly the moment that doesn't
// match reality — catching a firm-to-firm mismatch immediately instead of
// it silently processing zero emails forever with no obvious sign why. ──
function getClientEmailMonitoringStatus(){
  var configuredInbox=getSettingValue('clientEmailMonitoringInbox','');
  var actualInbox='';
  try{ actualInbox=Session.getEffectiveUser().getEmail(); }catch(e){ /* hidden by permission scope in some contexts — status still reported without it */ }
  var triggers=ScriptApp.getProjectTriggers();
  var triggerActive=triggers.some(function(t){return t.getHandlerFunction()==='checkClientEmailsForTasks';});
  var processedCount=0;
  try{
    var ss=SpreadsheetApp.openById(SHEET_ID);
    var sh=ss.getSheetByName(PROCESSED_EMAILS_TAB);
    if(sh) processedCount=Math.max(0, sh.getLastRow()-1);
  }catch(e2){}
  return {
    configuredInbox: configuredInbox,
    actualAuthorizedInbox: actualInbox,
    matches: !configuredInbox || !actualInbox || configuredInbox.toLowerCase()===actualInbox.toLowerCase(),
    triggerActive: triggerActive,
    emailsProcessedSoFar: processedCount
  };
}
...
if(data.action==='deleteLogById'){var lsh=ss.getSheetByName(LOG_TAB);if(lsh&&lsh.getLastRow()>1){var rid=String(data.rowId||'').trim();var dmob=String(data.mobile||'').trim();var dts=String(data.ts||'').trim();var dact=String(data.activity||'').trim();var n=lsh.getLastRow();var rng=lsh.getRange(2,1,n-1,7).getValues();for(var rj=0;rj<rng.length;rj++){var row=rng[rj];var gId=String(row[6]||'').trim();var rTs=String(row[0]||'').trim();var rMob=String(row[2]||'').trim();var rAct=String(row[4]||'').trim();var matchById=(rid!==''&&gId===rid);var matchByTs=(rid===''||gId==='')&&dts!==''&&rMob===dmob&&rTs===dts&&rAct===dact;if(matchById||matchByTs){lsh.deleteRow(rj+2);break;}}}return out({success:true,deleted:true});}if(data.action==='deleteLog'){var lsh2=ss.getSheetByName(LOG_TAB);if(lsh2&&lsh2.getLastRow()>1){var mob3=String(data.mobile||'').trim();var n2=lsh2.getLastRow();for(var rk=n2;rk>=2;rk--){if(String(lsh2.getRange(rk,3).getValue()||'').trim()===mob3){lsh2.deleteRow(rk);}}}return out({success:true});}
  ...
  // ═══════════════════════════════════════════════
  // FIRM BRANDING — Settings sheet, simple Key/Value pairs (row per key).
  // Used to white-label the app: firm name + logo shown on the client-facing
  // PWA pages (feedback, review, upload, gatherinfo, findoc) instead of the
  // fixed emoji, so this same deployment can be resold to a different firm
  // without editing every HTML file by hand. The logo itself is stored as a
  // data: URI directly in the sheet cell (not Drive) so it can be handed back
  // to those public, unauthenticated pages with zero sharing/permission setup.
  // Sheet cells cap out around 50,000 characters, so keep the source image
  // small — a square logo under ~30KB is plenty and leaves headroom.
  // ═══════════════════════════════════════════════
  function getSettingValue(key, def){
    var gsSh=ss.getSheetByName(SETTINGS_TAB);
    if(!gsSh || gsSh.getLastRow()<2) return def;
    var gsVals=gsSh.getRange(2,1,gsSh.getLastRow()-1,2).getValues();
    for(var i=0;i<gsVals.length;i++){
      if(String(gsVals[i][0]||'').trim()===key){ var v=String(gsVals[i][1]||''); return v||def; }
    }
    return def;
  }
  function setSettingValue(key, value){
    var ssSh=ss.getSheetByName(SETTINGS_TAB)||ss.insertSheet(SETTINGS_TAB);
    if(ssSh.getLastRow()===0){
      var ssHr=ssSh.getRange(1,1,1,2);
      ssHr.setValues([['Key','Value']]);
      ssHr.setFontWeight('bold').setBackground('#C0392B').setFontColor('#FFFFFF');
      ssSh.setFrozenRows(1);
    }
    var ssLr=ssSh.getLastRow();
    if(ssLr>1){
      var ssKeys=ssSh.getRange(2,1,ssLr-1,1).getValues();
      for(var i=0;i<ssKeys.length;i++){
        if(String(ssKeys[i][0]||'').trim()===key){ ssSh.getRange(i+2,2).setValue(value); return; }
      }
    }
    ssSh.appendRow([key, value]);
  }

  // Public — the client-facing PWA pages call this with no login, so it must
  // never require auth and must never return anything sensitive.
  if(data.action==='getFirmBranding'){
    try{
      return out({success:true, firmName:getSettingValue('firmName',''), logoDataUrl:getSettingValue('logoDataUrl','')});
    }catch(e){ return out({success:false, error:e.message}); }
  }

  // Admin only — sets the firm name and/or logo shown across the app and the
  // client-facing pages. Either field can be sent alone (e.g. renaming the
  // firm without re-uploading the logo).
  if(data.action==='saveFirmBranding'){
    try{
      if(!verifyIsAdmin(data.requestingUserId)) return out({success:false, error:'Not authorized'});
      if(data.firmName!==undefined) setSettingValue('firmName', String(data.firmName||'').trim());
      if(data.logoBase64 && data.mimeType){
        var fbDataUrl='data:'+data.mimeType+';base64,'+data.logoBase64;
        if(fbDataUrl.length>49000) return out({success:false, error:'Logo is too large — please use a smaller image (under ~30KB).'});
        setSettingValue('logoDataUrl', fbDataUrl);
      }
      return out({success:true, firmName:getSettingValue('firmName',''), logoDataUrl:getSettingValue('logoDataUrl','')});
    }catch(e){ return out({success:false, error:e.message}); }
  }

  // Generic single key/value settings — used for anything that doesn't
  // warrant its own dedicated action (e.g. the PWA's own hosting base URL,
  // captured automatically from the browser the first time the in-app
  // Health Check runs, and read back by the server-side diagnostic below
  // since Apps Script has no browser location to read it from directly).
  if(data.action==='getAppSetting'){
    try{ return out({success:true, key:data.key, value:getSettingValue(String(data.key||''), '')}); }
    catch(e){ return out({success:false, error:e.message}); }
  }
  if(data.action==='setAppSetting'){
    try{
      if(!data.key) return out({success:false, error:'No key given'});
      setSettingValue(String(data.key), String(data.value||''));
      return out({success:true});
    }catch(e){ return out({success:false, error:e.message}); }
  }

  // ── Sheet identity — exists because of a real incident: SHEET_ID pointed
  // at the wrong spreadsheet for a stretch with zero visible symptoms
  // beyond "login doesn't work", since every read/write silently succeeded
  // against the WRONG sheet. getSheetIdentity reports which spreadsheet
  // the backend is actually connected to right now; confirmSheetIdentity
  // lets an admin explicitly "lock in" that ID as correct, once, so every
  // future diagnostic run can catch a mismatch automatically instead of
  // relying on someone noticing the data looks off. ──
  if(data.action==='getSheetIdentity'){
    try{ return out({success:true, sheetId:SHEET_ID, sheetName:ss.getName(), sheetUrl:ss.getUrl(), confirmedSheetId:getSettingValue('confirmedSheetId','')}); }
    catch(e){ return out({success:false, error:e.message}); }
  }
  if(data.action==='confirmSheetIdentity'){
    try{
      if(!verifyIsAdmin(data.requestingUserId)) return out({success:false, error:'Not authorized'});
      setSettingValue('confirmedSheetId', SHEET_ID);
      return out({success:true, confirmedSheetId:SHEET_ID});
    }catch(e){ return out({success:false, error:e.message}); }
  }

  // ═══════════════════════════════════════════════
  // CALENDAR RECONNECT — a full "complete reconnect" of the calendar
  // integration. Previously the app silently assumed the script owner's
  // DEFAULT calendar was the right one to read client meetings/events
  // from — exactly the same class of bug as the earlier wrong-sheet
  // incident: technically working, just quietly pointed at the wrong
  // resource. This replaces that assumption with an explicit choice: list
  // every calendar the script can actually see, let an admin pick the
  // real one, and store that choice so getCalendarEvents below uses it
  // instead of guessing. Falls back to the default calendar only when
  // nothing has been explicitly chosen yet, for backward compatibility.
  // ═══════════════════════════════════════════════
  if(data.action==='listAvailableCalendars'){
    try{
      var allCalsForList=CalendarApp.getAllCalendars();
      var defaultCalId=CalendarApp.getDefaultCalendar().getId();
      var calList=allCalsForList.map(function(c){
        return {id:c.getId(), name:c.getName(), isDefault:(c.getId()===defaultCalId)};
      });
      return out({success:true, calendars:calList, selectedCalendarId:getSettingValue('selectedCalendarId','')});
    }catch(e){ return out({success:false, error:e.message}); }
  }
  if(data.action==='saveCalendarSelection'){
    try{
      if(!verifyIsAdmin(data.requestingUserId)) return out({success:false, error:'Not authorized'});
      if(!data.calendarId) return out({success:false, error:'No calendar given'});
      // Verify it's actually reachable before saving — never lock in a
      // choice that turns out to be unreadable.
      var testCal=CalendarApp.getCalendarById(data.calendarId);
      if(!testCal) return out({success:false, error:'Could not open that calendar — it may not be accessible to this script'});
      setSettingValue('selectedCalendarId', data.calendarId);
      setSettingValue('selectedCalendarName', testCal.getName());
      return out({success:true, calendarName:testCal.getName()});
    }catch(e){ return out({success:false, error:e.message}); }
  }

  // ═══════════════════════════════════════════════
  // MULTIPLE CONNECTED CALENDARS (up to 3) — extends the single-calendar
  // selection above to support pulling events/birthdays from several
  // Google accounts' calendars at once, e.g. a few advisors' own
  // calendars feeding into one shared "Today's Activity" view.
  //
  // IMPORTANT — this is a hard constraint of Google's own security model,
  // not a limitation of this app: Apps Script can only read a calendar
  // that has been EXPLICITLY SHARED with the account this script runs
  // as. There is no way to grant access just by typing an email address
  // in — the calendar's owner must go into Google Calendar's own
  // "Settings and sharing" for that calendar and share it with this
  // script's account first. getConnectedCalendarsInfo reports that
  // account's email so the UI can tell each person exactly who to share
  // with, and saveConnectedCalendars verifies each entry live so a typo
  // or a not-yet-shared calendar is caught immediately rather than
  // silently missing events forever.
  // ═══════════════════════════════════════════════
  if(data.action==='getConnectedCalendarsInfo'){
    try{
      var scriptAccountEmail='';
      try{ scriptAccountEmail=Session.getEffectiveUser().getEmail(); }catch(permErr){ /* email hidden by permission scope — still usable, just can't display it */ }
      var savedList=[];
      try{ savedList=JSON.parse(getSettingValue('connectedCalendars','[]'))||[]; }catch(parseErr){ savedList=[]; }
      return out({success:true, scriptAccountEmail:scriptAccountEmail, calendars:savedList});
    }catch(e){ return out({success:false, error:e.message}); }
  }
  if(data.action==='saveConnectedCalendars'){
    try{
      if(!verifyIsAdmin(data.requestingUserId)) return out({success:false, error:'Not authorized'});
      var emailsIn=Array.isArray(data.emails)?data.emails:[];
      var emails=emailsIn.map(function(e){return String(e||'').trim();}).filter(function(e){return e;}).slice(0,3);
      var scriptAccountEmailForErr='';
      try{ scriptAccountEmailForErr=Session.getEffectiveUser().getEmail(); }catch(e0){}
      var results=emails.map(function(email){
        try{
          var cal=CalendarApp.getCalendarById(email);
          if(cal){
            // getCalendarById can return a non-null object even for an
            // inaccessible calendar in some cases — actually try to read
            // from it, which is the real test of whether sharing worked.
            cal.getName();
            return {email:email, ok:true, name:cal.getName()};
          }
          return {email:email, ok:false, name:'', error:'Not shared with '+(scriptAccountEmailForErr||'this script\'s account')+' yet'};
        }catch(e1){
          return {email:email, ok:false, name:'', error:'Not accessible \u2014 share this calendar with '+(scriptAccountEmailForErr||'this script\'s account')+' first (Google Calendar \u2192 Settings and sharing \u2192 Share with specific people)'};
        }
      });
      // Save every entry the admin typed (not just the successful ones) so
      // nothing is silently dropped from the list — a calendar shared a
      // few minutes AFTER saving will start working on the next fetch
      // without needing to be re-typed, and a failed one stays visible as
      // a reminder rather than vanishing.
      setSettingValue('connectedCalendars', JSON.stringify(results.map(function(r){return {email:r.email, name:r.name};})));
      return out({success:true, scriptAccountEmail:scriptAccountEmailForErr, results:results});
    }catch(e){ return out({success:false, error:e.message}); }
  }

  // ═══════════════════════════════════════════════
  // SERVER-SIDE DIAGNOSTIC + AUTO-EMAIL — runs the whole check suite
  // entirely within this one Apps Script execution and emails the report
  // at the end via MailApp, deliberately NOT depending on the browser tab
  // that triggered it staying open. Apps Script executions triggered by a
  // web request keep running on Google's servers once started, independent
  // of whether the client is still listening for the HTTP response — that
  // is what makes "close the app right after tapping the button, still get
  // the email a minute later" work. Internal checks call writeData(...)
  // directly (a plain function call, not a second HTTP round trip) so this
  // reuses the exact same tested read/write logic as every real action,
  // rather than a second, parallel implementation that could drift out of
  // sync with it over time.
  // ═══════════════════════════════════════════════
  if(data.action==='runServerDiagnosticAndEmail'){
    try{
      var rPass=0, rFail=0, rWarn=0;
      var rpt=[];
      function rlog(m){ rpt.push(m); }
      function rok(l){ rPass++; rlog('  \u2705 '+l); }
      function rno(l){ rFail++; rlog('  \u274C '+l); }
      function rwn(l){ rWarn++; rlog('  \u26A0\uFE0F '+l); }
      function callAction(payload){
        try{ return JSON.parse(writeData(payload).getContent()); }
        catch(e){ return {success:false, error:e.message}; }
      }
      // Builds a row array matched by HEADER TEXT rather than a hardcoded
      // column position — same tolerant approach the rest of the app uses,
      // so this test can't silently give a wrong result just because a
      // column got reordered or a header was edited.
      function buildRowByHeaders(sh, valuesByKeyword){
        var lastCol=Math.max(sh.getLastColumn(),1);
        var hdr=sh.getRange(1,1,1,lastCol).getValues()[0].map(function(h){return (h||'').toString().toLowerCase().trim();});
        var row=new Array(lastCol).fill('');
        Object.keys(valuesByKeyword).forEach(function(kw){
          for(var i=0;i<hdr.length;i++){ if(hdr[i].indexOf(kw)>=0){ row[i]=valuesByKeyword[kw]; break; } }
        });
        return row;
      }

      var startTime=new Date();
      rlog('Deep Pocket CRM \u2014 Automated Diagnostic Report');
      rlog('Runs entirely on the server \u2014 completes and emails this report even if the app was closed right after starting it.');
      rlog('Started: '+startTime.toString());
      rlog('');

      // PRE-CHECK: SHEET IDENTITY — comes before everything else,
      // deliberately. A real incident is why this exists: the backend was
      // silently connected to the wrong spreadsheet for a stretch, and
      // every check below would have reported "working correctly" the
      // whole time — just against the wrong sheet. If this fails, nothing
      // else in this report can be trusted until it's fixed.
      rlog('\u2501\u2501 SHEET IDENTITY \u2501\u2501');
      var sheetMismatchDetected=false;
      try{
        rok('Connected sheet: "'+ss.getName()+'"');
        rlog('     ID: '+SHEET_ID);
        rlog('     '+ss.getUrl());
        var confirmedId=getSettingValue('confirmedSheetId','');
        if(!confirmedId){
          rwn('No confirmed sheet ID set yet \u2014 in the app, open Settings, paste your Google Sheet URL, and tap "Verify & Save" once, so future runs (including this automated one) can catch a mismatch automatically');
        } else if(confirmedId===SHEET_ID){
          rok('Matches the confirmed correct sheet');
        } else {
          sheetMismatchDetected=true;
          rno('MISMATCH \u2014 backend is connected to a DIFFERENT sheet than the one confirmed correct. Confirmed: '+confirmedId+' \u2014 Currently connected: '+SHEET_ID+'. Check SHEET_ID in Code.gs.');
        }
      }catch(e){ rno('Sheet identity check error: '+e.message); }
      rlog('');

      // 0. Backend version + firm
      rlog('\u2501\u2501 0. Backend & firm \u2501\u2501');
      rok('Backend version: '+VERSION);
      var dFbName=getSettingValue('firmName','');
      var dFbLogo=getSettingValue('logoDataUrl','');
      if(dFbName) rok('Firm name: '+dFbName); else rwn('No firm name set (client-facing pages show the default placeholder)');
      if(dFbLogo) rok('Firm logo is set'); else rwn('No firm logo set (client-facing pages show the default emoji)');
      rlog('');

      // 1. Connection
      rlog('\u2501\u2501 1. Connection \u2501\u2501');
      var leadsSh, clientsSh;
      try{
        leadsSh=ss.getSheetByName(LEADS_TAB);
        clientsSh=ss.getSheetByName(CLIENTS_TAB);
        var lCount=leadsSh?Math.max(0,leadsSh.getLastRow()-1):0;
        var cCount=clientsSh?Math.max(0,clientsSh.getLastRow()-1):0;
        if(leadsSh && clientsSh) rok('Sheet connection working \u2014 '+lCount+' leads, '+cCount+' clients');
        else rno('LEADS or CLIENTS tab not found \u2014 check tab names are exactly "LEADS" and "CLIENTS"');
      }catch(e){ rno('Sheet connection failed: '+e.message); }
      rlog('');

      // 2. Leads CRUD
      rlog('\u2501\u2501 2. Leads (add \u00B7 update \u00B7 delete) \u2501\u2501');
      var SD_MOB='9000000019';
      try{
        callAction({action:'deleteLead', mobile:SD_MOB});
        var lRow=buildRowByHeaders(leadsSh, {name:'SERVER DIAG TEST', mobile:SD_MOB, status:'PROSPECT', source:'SERVERDIAG'});
        var addRes=callAction({action:'appendLead', row:lRow});
        Utilities.sleep(400);
        var lData=leadsSh.getDataRange().getValues();
        var lFoundRow=lData.find(function(r){return String(r[1]||'').trim()===SD_MOB;});
        if(lFoundRow) rok('Lead add works \u2014 found in sheet');
        else rno('Lead add failed \u2014 not found in sheet after write');
        if(lFoundRow && addRes && addRes.recordId){
          var lHdrRow=leadsSh.getRange(1,1,1,leadsSh.getLastColumn()).getValues()[0];
          var statusColIdx=lHdrRow.findIndex(function(h){return (h||'').toString().toLowerCase().indexOf('status')>=0;});
          var lUpdRow=lFoundRow.slice();
          if(statusColIdx>=0) lUpdRow[statusColIdx]='CONTACTED';
          callAction({action:'updateLead', mobile:SD_MOB, recordId:addRes.recordId, row:lUpdRow});
          Utilities.sleep(400);
          var lData2=leadsSh.getDataRange().getValues();
          var lFound2=lData2.find(function(r){return String(r[1]||'').trim()===SD_MOB;});
          if(lFound2 && statusColIdx>=0 && String(lFound2[statusColIdx]||'').trim().toUpperCase()==='CONTACTED') rok('Lead update works \u2014 change landed in sheet');
          else rno('Lead update did not reflect in sheet');
        }
        callAction({action:'deleteLead', mobile:SD_MOB, recordId:(addRes&&addRes.recordId)||''});
        Utilities.sleep(400);
        var lData3=leadsSh.getDataRange().getValues();
        if(!lData3.some(function(r){return String(r[1]||'').trim()===SD_MOB;})) rok('Lead delete works \u2014 row removed');
        else rno('Lead delete failed \u2014 row still present');
      }catch(e){ rno('Leads CRUD error: '+e.message); try{ callAction({action:'deleteLead', mobile:SD_MOB}); }catch(e2){} }
      rlog('');

      // 3. Clients CRUD
      rlog('\u2501\u2501 3. Clients (add \u00B7 update \u00B7 delete) \u2501\u2501');
      var SD_CMOB='9000000029';
      try{
        callAction({action:'deleteClient', mobile:SD_CMOB});
        var cRow=buildRowByHeaders(clientsSh, {name:'SERVER DIAG TEST', mobile:SD_CMOB, stage:'ACTIVE'});
        var cAddRes=callAction({action:'appendClient', row:cRow});
        Utilities.sleep(400);
        var cData=clientsSh.getDataRange().getValues();
        var cFoundRow=cData.find(function(r){return String(r[1]||'').trim()===SD_CMOB;});
        if(cFoundRow) rok('Client add works \u2014 found in sheet');
        else rno('Client add failed \u2014 not found in sheet after write');
        if(cFoundRow){
          var cHdrRow=clientsSh.getRange(1,1,1,clientsSh.getLastColumn()).getValues()[0];
          var stageColIdx=cHdrRow.findIndex(function(h){return (h||'').toString().toLowerCase().indexOf('stage')>=0;});
          var cUpdRow=cFoundRow.slice();
          if(stageColIdx>=0) cUpdRow[stageColIdx]='DOCUMENT PENDING';
          callAction({action:'updateClient', mobile:SD_CMOB, row:cUpdRow});
          Utilities.sleep(400);
          var cData2=clientsSh.getDataRange().getValues();
          var cFound2=cData2.find(function(r){return String(r[1]||'').trim()===SD_CMOB;});
          if(cFound2 && stageColIdx>=0 && String(cFound2[stageColIdx]||'').trim().toUpperCase()==='DOCUMENT PENDING') rok('Client update works \u2014 change landed in sheet');
          else rno('Client update did not reflect in sheet');
        }
        callAction({action:'deleteClient', mobile:SD_CMOB});
        Utilities.sleep(400);
        var cData3=clientsSh.getDataRange().getValues();
        if(!cData3.some(function(r){return String(r[1]||'').trim()===SD_CMOB;})) rok('Client delete works \u2014 row removed');
        else rno('Client delete failed \u2014 row still present');
      }catch(e){ rno('Clients CRUD error: '+e.message); try{ callAction({action:'deleteClient', mobile:SD_CMOB}); }catch(e2){} }
      rlog('');

      // 4. Activity log
      rlog('\u2501\u2501 4. Activity log \u2501\u2501');
      try{
        // appendLog expects a full row array [Timestamp, Name, Mobile, Type,
        // Activity, Notes, Raw ID, Record ID, Logged By] — NOT named params,
        // matching how the sheet itself is laid out (same pattern as
        // appendLead/appendClient below).
        var logRawId='SERVERDIAG-'+Date.now();
        var logRow=[new Date().toString(), 'SERVER DIAG TEST', SD_MOB, 'lead', 'Server diagnostic test entry', '', logRawId, '', 'diagnostic'];
        var logRes=callAction({action:'appendLog', row:logRow});
        if(logRes && logRes.success && !logRes.skipped) rok('Activity log add works');
        else if(logRes && logRes.skipped) rwn('Activity log reported a duplicate on a fresh Raw ID \u2014 unexpected, worth a manual look');
        else rno('Activity log add failed: '+(logRes&&logRes.error||'unknown error'));
      }catch(e){ rno('Activity log error: '+e.message); }
      rlog('');

      // 5. Drive folder
      rlog('\u2501\u2501 5. Drive folder \u2501\u2501');
      try{
        var folderRes=callAction({action:'createClientFolder', clientName:'SERVER DIAG TEST', mobile:SD_CMOB});
        if(folderRes && folderRes.success && folderRes.folderId){
          rok('Client Drive folder create/verify works');
          try{ DriveApp.getFolderById(folderRes.folderId).setTrashed(true); }catch(delErr){ rwn('Could not clean up the test Drive folder \u2014 harmless, just a leftover test folder to delete manually'); }
        } else {
          rno('Drive folder creation failed: '+(folderRes&&folderRes.error||'unknown error'));
        }
      }catch(e){ rno('Drive folder error: '+e.message); }
      rlog('');

      // 6. Calendar & birthdays
      rlog('\u2501\u2501 6. Calendar & birthday integration \u2501\u2501');
      try{
        var cal=CalendarApp.getDefaultCalendar();
        rok('Default calendar: '+cal.getName());
        var selCalIdDiag=getSettingValue('selectedCalendarId','');
        if(selCalIdDiag){
          try{
            var selCalDiag=CalendarApp.getCalendarById(selCalIdDiag);
            if(selCalDiag) rok('Explicitly selected calendar (used for events): "'+selCalDiag.getName()+'"');
            else rno('A calendar was selected but is no longer reachable \u2014 re-open Settings and pick again');
          }catch(selDiagErr){ rno('Selected calendar error: '+selDiagErr.message); }
        } else {
          rwn('No calendar explicitly selected yet \u2014 currently falling back to the default above. Open Settings and use "Load My Calendars" to pick the real one.');
        }
        var todayD=new Date(); var tmrD=new Date(todayD.getTime()+86400000);
        var evRes=callAction({action:'getCalendarEvents', start:Utilities.formatDate(todayD,'Asia/Kolkata','yyyy-MM-dd'), end:Utilities.formatDate(todayD,'Asia/Kolkata','yyyy-MM-dd')});
        if(evRes){
          var bCount=evRes.birthdaysFound||0;
          rok('Today: '+((evRes.events||[]).filter(function(e){return e.type!=='birthday';}).length)+' event(s), '+bCount+' birthday(s) \u2014 source: '+(evRes.birthdaySource||'none'));
          if(evRes.calendarsUsed && evRes.calendarsUsed.length) rok('Calendars actually read from: '+evRes.calendarsUsed.join(', '));
        }
        try{
          var connList=JSON.parse(getSettingValue('connectedCalendars','[]'))||[];
          if(connList.length){
            connList.forEach(function(c){
              if(c.name) rok('Connected calendar "'+c.email+'" \u2014 reachable ("'+c.name+'")');
              else rno('Connected calendar "'+c.email+'" \u2014 NOT reachable, share it with the script\'s account (see Settings for the exact email)');
            });
          } else {
            rwn('No additional connected calendars configured \u2014 add up to 3 in Settings if events from other Google accounts should show here too');
          }
        }catch(connErr){ rwn('Connected calendars check error: '+connErr.message); }
      }catch(e){ rwn('Calendar check error: '+e.message); }
      rlog('');

      // 7. Quick Reminders
      rlog('\u2501\u2501 7. Quick Reminders \u2501\u2501');
      try{
        var remDate=Utilities.formatDate(new Date(Date.now()+7*86400000),'Asia/Kolkata','yyyy-MM-dd');
        var remAdd=callAction({action:'addReminder', date:Utilities.formatDate(new Date(),'Asia/Kolkata','yyyy-MM-dd'), clientName:'SERVER DIAG TEST', reminderText:'diagnostic test', reminderDate:remDate});
        if(remAdd && remAdd.success) rok('Reminder add works');
        else rno('Reminder add failed: '+(remAdd&&remAdd.error||'unknown error'));
        callAction({action:'deleteReminder', date:Utilities.formatDate(new Date(),'Asia/Kolkata','yyyy-MM-dd'), clientName:'SERVER DIAG TEST', reminderText:'diagnostic test', reminderDate:remDate});
      }catch(e){ rno('Reminders error: '+e.message); }
      rlog('');

      // 8. AI API connectivity
      rlog('\u2501\u2501 8. AI API connectivity (Gemini) \u2501\u2501');
      try{
        var chatRes=callAction({action:'practiceChat', message:'Reply with exactly the word: OK', history:[]});
        if(chatRes && chatRes.reply) rok('Insights AI chat responds \u2014 Gemini connection working');
        else rno('Insights AI chat failed: '+(chatRes&&chatRes.error||'no reply \u2014 check GEMINI_API_KEY in Script Properties'));
      }catch(e){ rno('AI chat error: '+e.message); }
      try{
        var vcRes=callAction({action:'interpretVoiceCommand', command:'add a lead named Test User mobile 9999999999', languageName:'English', candidateNames:[]});
        if(vcRes && vcRes.success && vcRes.result && vcRes.result.actions && vcRes.result.actions.length) rok('AI Voice Assistant interpreter responds correctly');
        else rno('AI Voice Assistant interpreter failed: '+(vcRes&&vcRes.error||'no valid response'));
      }catch(e){ rno('AI Voice Assistant error: '+e.message); }
      rlog('');

      // 9. External pages — needs the PWA's own hosting URL, which Apps
      // Script has no way to know on its own (there's no browser here to
      // ask). Reads it from a setting the in-app Health Check saves
      // automatically the first time someone runs it from a real browser.
      rlog('\u2501\u2501 9. External pages (client-facing links) \u2501\u2501');
      var pwaBase=getSettingValue('pwaBaseUrl','');
      if(!pwaBase){
        rwn('PWA base URL not known yet \u2014 run the in-app Health Check once from a browser first (it saves this automatically), then this section will work here too');
      } else {
        var extPagesDiag=['feedback.html','upload.html','gatherinfo.html','findoc.html','review.html'];
        extPagesDiag.forEach(function(pg){
          try{
            var pgRes=UrlFetchApp.fetch(pwaBase+pg, {muteHttpExceptions:true});
            var pgCode=pgRes.getResponseCode();
            var pgBody=pgRes.getContentText();
            if(pgCode===200 && (pgBody.indexOf('<html')>=0 || pgBody.indexOf('<!DOCTYPE')>=0)) rok(pg+' loads correctly (HTTP '+pgCode+')');
            else rno(pg+' returned HTTP '+pgCode+' or unexpected content');
          }catch(e){ rno(pg+' failed to load: '+e.message); }
        });
      }
      rlog('');

      // 10. Field → column header matching
      rlog('\u2501\u2501 10. Field \u2192 column header matching \u2501\u2501');
      try{
        var lHdr2=(leadsSh.getRange(1,1,1,leadsSh.getLastColumn()).getValues()[0]||[]).map(function(h){return (h||'').toString().toLowerCase().trim();});
        var lci=function(keys){for(var ki=0;ki<keys.length;ki++)for(var hi=0;hi<lHdr2.length;hi++)if(lHdr2[hi].indexOf(keys[ki])>=0)return hi;return -1;};
        var lFieldsD={name:['name'],mobile:['mobile','phone'],email:['email'],status:['status'],source:['source'],'record id':['record id']};
        var lMiss2=Object.keys(lFieldsD).filter(function(f){return lci(lFieldsD[f])<0;});
        if(!lMiss2.length) rok('All expected LEADS columns found by header text');
        else rno('LEADS columns not found by header text: '+lMiss2.join(', '));

        var cHdr2=(clientsSh.getRange(1,1,1,clientsSh.getLastColumn()).getValues()[0]||[]).map(function(h){return (h||'').toString().toLowerCase().trim();});
        var cci=function(keys){for(var ki=0;ki<keys.length;ki++)for(var hi=0;hi<cHdr2.length;hi++)if(cHdr2[hi].indexOf(keys[ki])>=0)return hi;return -1;};
        var cFieldsD={name:['name'],mobile:['mobile','phone'],email:['email'],stage:['stage'],'record id':['record id']};
        var cMiss2=Object.keys(cFieldsD).filter(function(f){return cci(cFieldsD[f])<0;});
        if(!cMiss2.length) rok('All expected CLIENTS columns found by header text');
        else rno('CLIENTS columns not found by header text: '+cMiss2.join(', '));
      }catch(e){ rno('Field/column matching error: '+e.message); }
      rlog('');

      // 11. Central feedback sheet — app/product feedback from the firms
      // Deep Pocket sells this CRM to, ONLY. Retail client feedback (about
      // their own advisor) is local to each firm's own sheet and never
      // touches this central one.
      rlog('\u2501\u2501 11. Central feedback sheet (app/product feedback) \u2501\u2501');
      try{
        var cfsSs=SpreadsheetApp.openById(CENTRAL_FEEDBACK_SHEET_ID);
        rok('Reachable \u2014 "'+cfsSs.getName()+'"');
        var cfsSh=cfsSs.getSheetByName('FEEDBACK');
        if(cfsSh) rok('FEEDBACK tab exists ('+Math.max(0,cfsSh.getLastRow()-1)+' submission(s) so far)');
        else rwn('FEEDBACK tab doesn\'t exist yet \u2014 it will be created automatically on the first product-feedback submission');
      }catch(e){ rno('Could not reach the central feedback sheet \u2014 product feedback submissions will fail until this account is shared as an editor on it: '+e.message); }
      rlog('');

      // SUMMARY
      rlog('\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501');
      rlog('RESULT: '+rPass+' passed \u00B7 '+rFail+' failed \u00B7 '+rWarn+' warnings');
      rlog(rFail===0 ? '\uD83C\uDF89 All critical checks passed.' : 'Some checks failed \u2014 review the \u274C lines above.');
      rlog('');
      rlog('Finished: '+new Date().toString());

      var fullReport=rpt.join('\n');
      var emailSubject=(sheetMismatchDetected?'\u26A0\uFE0F SHEET MISMATCH \u2014 ':'')+'Deep Pocket CRM Diagnostic — '+(dFbName||'')+' — '+rPass+' passed, '+rFail+' failed, '+rWarn+' warnings';
      try{
        MailApp.sendEmail({
          to:'deeppocket1610@gmail.com',
          subject:emailSubject,
          body:fullReport
        });
      }catch(mailErr){
        // If the email itself fails to send (quota, invalid address, etc.)
        // that's still worth returning to whoever's watching the immediate
        // response, even though the whole point of this action is not
        // depending on that.
        return out({success:false, error:'Diagnostic completed but email failed to send: '+mailErr.message, report:fullReport});
      }

      return out({success:true, summary:rPass+' passed, '+rFail+' failed, '+rWarn+' warnings', emailedTo:'deeppocket1610@gmail.com'});
    }catch(e){
      // Even a hard failure here should still try to notify by email —
      // silent total failure would defeat the entire point of this action.
      try{ MailApp.sendEmail('deeppocket1610@gmail.com', 'Deep Pocket CRM Diagnostic — FAILED TO RUN', 'The server-side diagnostic crashed before completing: '+e.message); }catch(e2){}
      return out({success:false, error:e.message});
    }
  }

  // ═══════════════════════════════════════════════
  // AI VOICE ASSISTANT — turns a spoken command (already transcribed to text
  // in the browser via the Web Speech API) into structured intent + fields
  // using Gemini. Deliberately does NOT write anything itself — it only
  // returns structured JSON; the frontend decides what to do with it (opens
  // the matching form pre-filled, always leaving the actual Save/Confirm tap
  // to the advisor). Keeping the mutation client-side, on the same proven
  // form-save code paths already used everywhere else, avoids a second,
  // parallel way to write leads/clients/tasks that could drift out of sync
  // with the real one.
  // ═══════════════════════════════════════════════
  // ═══════════════════════════════════════════════
  // AUDIO TRANSCRIPTION FALLBACK — for devices/browsers where
  // SpeechRecognition doesn't work (iOS in particular: Apple requires every
  // iOS browser to use WebKit, and WebKit's speech recognition is either
  // absent or documented as unreliable). The frontend records raw audio
  // with MediaRecorder instead (a much older, better-supported API) and
  // sends it here as a single request — nothing is written to Drive or any
  // other persistent storage, the audio only ever exists in this one
  // request/response round trip. This returns ONLY the transcript, not an
  // interpreted command — the frontend then feeds that transcript into the
  // existing interpretVoiceCommand action, exactly as if it had come from
  // SpeechRecognition, so the confirm-before-committing-tokens flow and
  // candidate-name matching are unchanged either way.
  // ═══════════════════════════════════════════════
  if(data.action==='transcribeVoiceAudio'){
    try{
      if(!data.audioBase64 || !data.mimeType) return out({success:false, error:'No audio received'});
      var vtLangName=String(data.languageName||'English').trim()||'English';
      var vtPrompt='Transcribe this short audio recording verbatim into text, in '+vtLangName+'. The speaker is giving '
        +'a short spoken command to a CRM app (e.g. adding a lead, setting a reminder, or asking to open a client '
        +'record) \u2014 transcribe exactly what they said, do not interpret, summarize, translate, or add anything. '
        +'Respond with ONLY the transcript text itself \u2014 no quotes, no labels, no extra commentary. If the '
        +'recording contains no discernible speech, respond with exactly: (no speech detected)';
      var vtReply=callGemini({
        mixedContent:[
          {type:'document', source:{media_type:data.mimeType, data:data.audioBase64}},
          {type:'text', text:vtPrompt}
        ],
        maxOutputTokens:300
      });
      var transcript=(vtReply||'').trim();
      if(!transcript || transcript.toLowerCase().indexOf('no speech detected')>=0){
        return out({success:false, error:'Could not make out any speech \u2014 please try again or type your command'});
      }
      return out({success:true, transcript:transcript});
    }catch(e){
      return out({success:false, error:e.message});
    }
  }

  if(data.action==='interpretVoiceCommand'){
    try{
      var vcToday=Utilities.formatDate(new Date(),'Asia/Kolkata','yyyy-MM-dd');
      var VC_FALLBACK='Sorry, I could not find this. Please try again manually.';
      // Regional-language support — the browser transcribes speech in the
      // advisor's chosen language (Settings > App Settings > Voice
      // Language) and sends the language name along with the already-
      // transcribed text; Gemini itself is natively multilingual, so no
      // separate translation step is needed. It also writes "reply" back
      // in that same language for a natural voice experience, except the
      // fixed English fallback below, which is always exact and literal.
      var vcLangName=String(data.languageName||'English').trim()||'English';
      var vcTomorrow=Utilities.formatDate(new Date(new Date().getTime()+86400000),'Asia/Kolkata','yyyy-MM-dd');

      // Real lead/client names, sent up by the frontend from its already-
      // loaded data, so Gemini matches the spoken name against ACTUAL
      // records instead of guessing a name in isolation. This is what makes
      // matching robust to transcription slips, partial names ("Rahul" for
      // "Rahul Sharma"), and script/spelling variants across languages —
      // far better than a plain substring search after the fact. Capped
      // defensively so an unusually large practice never blows the prompt
      // past a sane size; if it's ever truncated, matching simply falls
      // back to "no confident match" for anything past the cap rather than
      // silently matching the wrong person.
      var vcCandidates=Array.isArray(data.candidateNames)?data.candidateNames.slice(0,1500):[];
      var vcCandidateBlock=vcCandidates.length
        ? ('\n\nEXISTING LEAD/CLIENT NAMES IN THIS CRM (for FIND_RECORD and ADD_TASK only \u2014 match the spoken name '
          +'against this real list, accounting for transcription slips, partial names, nicknames, and spelling/script '
          +'variants; do NOT match against ADD_LEAD, that is always a brand-new person not in this list):\n'
          +vcCandidates.join(', '))
        : '';

      var vcPrompt='You are a voice command interpreter for a wealth-management CRM app used in India. The advisor spoke '
        +'in '+vcLangName+' and their words were transcribed to text below \u2014 the transcription itself may be imperfect. '
        +'Convert it into STRICT JSON only \u2014 no markdown, no code fences, no extra text before or after. Today\'s date '
        +'is '+vcToday+' (India, IST).'+vcCandidateBlock+'\n\n'
        +'The advisor may ask for MORE THAN ONE thing in a single sentence (e.g. joined by "and", "then", "also") \u2014 '
        +'e.g. "add Priya as a lead and remind me to follow up next week" is TWO actions: an ADD_LEAD, then an ADD_TASK. '
        +'Split these into separate, ordered action objects. Most commands are a single action \u2014 only split when the '
        +'advisor genuinely asked for more than one distinct thing.\n\n'
        +'Each action has exactly one of these intents:\n'
        +'- "ADD_LEAD": advisor wants to add a new lead/prospect\n'
        +'- "ADD_TASK": advisor wants to create a reminder/task/to-do\n'
        +'- "FIND_RECORD": advisor wants to open, update, or look up an EXISTING lead or client by name\n'
        +'- "UNKNOWN": command does not clearly match any of the above, is too vague to act on, or is unrelated to '
        +'leads/clients/tasks entirely (if the WHOLE command is unrecognizable, return exactly one UNKNOWN action, never split it)\n\n'
        +'Respond with ONLY this exact JSON shape (use "" or [] for any field that does not apply \u2014 never null, never omit a key):\n'
        +'{"actions":[{"intent":"ADD_LEAD|ADD_TASK|FIND_RECORD|UNKNOWN","name":"","mobile":"","email":"","gender":"MALE|FEMALE|","status":"PROSPECT|CONTACTED|IN PROGRESS|CONVERTED|","taskDetails":"","dueDate":"YYYY-MM-DD","nameMatches":[],"reply":"one short natural sentence, in '+vcLangName+', said back to the advisor confirming what you understood, for THIS action only"}]}\n\n'
        +'Rules: "mobile" is digits only \u2014 the last 10 digits spoken, nothing else. Resolve relative dates ("tomorrow", '
        +'"next Monday", "in 3 days", or their equivalents in '+vcLangName+') into an exact YYYY-MM-DD using today\'s date '
        +'above; if a task is mentioned with no date at all, use today\'s date. "name" is the person\'s name AS SPOKEN. '
        +'If a later action in the same command refers to the same person as an earlier ADD_LEAD action (e.g. "add Priya '
        +'as a lead and remind me to call her tomorrow"), still put that person\'s spoken name in "name" for the later '
        +'action too \u2014 the app resolves that linkage itself, you do not need to. "taskDetails" should stay in its '
        +'original spoken form/script. For FIND_RECORD or ADD_TASK, "nameMatches" is an array of every name from the '
        +'EXISTING LEAD/CLIENT NAMES list above that could plausibly be who the advisor meant \u2014 include more than one '
        +'if genuinely ambiguous (e.g. two similar names on the list), include none if nothing on the list plausibly '
        +'matches (this is normal and expected for a brand-new person who was just added earlier in the same command), '
        +'and use the EXACT spelling from that list, not the spoken form. For ADD_LEAD, "nameMatches" is always []. '
        +'Never invent a name, number, or detail that was not actually said or is not on the candidate list. If intent '
        +'is "UNKNOWN", set "reply" to exactly: "'+VC_FALLBACK+'". Otherwise keep each "reply" under 20 words.\n\n'
        +'Examples (any of these spoken in Hindi, Malayalam, Tamil, Telugu, or Kannada should be recognized the same way):\n'
        +'"Add a new lead, Priya Nair, mobile 9876543210, interested in mutual funds" \u2192 '
        +'{"actions":[{"intent":"ADD_LEAD","name":"Priya Nair","mobile":"9876543210","email":"","gender":"FEMALE","status":"PROSPECT","taskDetails":"","dueDate":"","nameMatches":[],"reply":"Adding Priya Nair as a new lead \u2014 please check and save."}]}\n'
        +'"Remind me to call Rahul tomorrow about his SIP" (candidate list contains "Rahul Sharma") \u2192 '
        +'{"actions":[{"intent":"ADD_TASK","name":"Rahul","mobile":"","email":"","gender":"","status":"","taskDetails":"Call Rahul about his SIP","dueDate":"'+vcTomorrow+'","nameMatches":["Rahul Sharma"],"reply":"Task set \u2014 call Rahul tomorrow about his SIP."}]}\n'
        +'"Open Sanjay\'s client profile" (candidate list contains both "Sanjay Mehta" and "Sanjay Kumar") \u2192 '
        +'{"actions":[{"intent":"FIND_RECORD","name":"Sanjay","mobile":"","email":"","gender":"","status":"","taskDetails":"","dueDate":"","nameMatches":["Sanjay Mehta","Sanjay Kumar"],"reply":"Found two people named Sanjay \u2014 please pick one."}]}\n'
        +'"Add Priya Nair as a new lead, mobile 9876543210, and remind me to follow up with her next week" \u2192 '
        +'{"actions":['
        +'{"intent":"ADD_LEAD","name":"Priya Nair","mobile":"9876543210","email":"","gender":"FEMALE","status":"PROSPECT","taskDetails":"","dueDate":"","nameMatches":[],"reply":"Adding Priya Nair as a new lead \u2014 please check and save."},'
        +'{"intent":"ADD_TASK","name":"Priya Nair","mobile":"","email":"","gender":"","status":"","taskDetails":"Follow up with Priya Nair","dueDate":"'+vcTomorrow+'","nameMatches":[],"reply":"Once she\'s saved, I\'ll set a follow-up task for next week."}'
        +']}\n'
        +'"What\'s the weather today" \u2192 '
        +'{"actions":[{"intent":"UNKNOWN","name":"","mobile":"","email":"","gender":"","status":"","taskDetails":"","dueDate":"","nameMatches":[],"reply":"'+VC_FALLBACK+'"}]}'
        +'\n\nCommand: "'+String(data.command||'').replace(/"/g,"'")+'"';

      var vcRaw=callGeminiFlashLite(vcPrompt, 700);
      var vcMatch=vcRaw.match(/\{[\s\S]*\}/);
      var vcParsed;
      try{ vcParsed=JSON.parse(vcMatch?vcMatch[0]:vcRaw); }
      catch(parseErr){
        // Could not even get valid JSON back — treat exactly like an
        // unrecognized command rather than a system error, per spec.
        return out({success:true, result:{actions:[{intent:'UNKNOWN', name:'', mobile:'', email:'', gender:'', status:'', taskDetails:'', dueDate:'', nameMatches:[], reply:VC_FALLBACK}]}});
      }
      // Defensive normalization: guarantee a non-empty actions array no
      // matter what shape came back (a stray flat object from an older
      // prompt version, a missing key, etc.) — the frontend only ever
      // reads result.actions.
      var vcActions=Array.isArray(vcParsed&&vcParsed.actions) ? vcParsed.actions : (vcParsed&&vcParsed.intent ? [vcParsed] : []);
      if(!vcActions.length){
        vcActions=[{intent:'UNKNOWN', name:'', mobile:'', email:'', gender:'', status:'', taskDetails:'', dueDate:'', nameMatches:[], reply:VC_FALLBACK}];
      }
      vcActions=vcActions.slice(0,3).map(function(a){
        a=a||{};
        // Server-side safety net: guarantee the exact fallback wording every
        // time, regardless of what Gemini actually wrote for "reply" — never
        // trust a model to reproduce a fixed string verbatim on every call.
        if(String(a.intent||'').toUpperCase()==='UNKNOWN'){
          a.intent='UNKNOWN';
          a.reply=VC_FALLBACK;
        }
        // Never let a missing/malformed nameMatches reach the frontend as
        // anything but a real array — a stray non-array value would break
        // the picker logic client-side. Also strip any hallucinated match
        // that isn't actually on the candidate list sent up.
        if(!Array.isArray(a.nameMatches)) a.nameMatches=[];
        if(vcCandidates.length) a.nameMatches=a.nameMatches.filter(function(n){ return vcCandidates.indexOf(n)!==-1; });
        return a;
      });
      return out({success:true, result:{actions:vcActions}});
    }catch(e){
      return out({success:false, error:e.message});
    }
  }
...
if(data.action==='tagLeadFolder'){
  ...
  if(data.action==='getCalendarEvents'){
    try{
      ...
      // Gather every calendar to actually read from: the new multi-calendar
      // list (up to 3, e.g. several advisors' own calendars) plus the
      // older single-selection setting for backward compatibility with
      // whatever was already configured before multi-calendar support
      // existed, falling back to the default calendar only if nothing at
      // all has been configured. Deduplicated so the same calendar never
      // gets queried (and its events counted) twice.
      var calIdsToFetch=[];
      try{
        var connectedList=JSON.parse(getSettingValue('connectedCalendars','[]'))||[];
        connectedList.forEach(function(c){ if(c && c.email) calIdsToFetch.push(c.email); });
      }catch(parseErr2){}
      var legacySelectedId=getSettingValue('selectedCalendarId','');
      if(legacySelectedId && calIdsToFetch.indexOf(legacySelectedId)<0) calIdsToFetch.push(legacySelectedId);

      var evRows=[];
      var calendarsUsed=[];
      if(calIdsToFetch.length){
        calIdsToFetch.forEach(function(calId){
          try{
            var c=CalendarApp.getCalendarById(calId);
            if(!c) return;
            var cName=c.getName();
            calendarsUsed.push(cName);
            c.getEvents(evStart,evEnd).forEach(function(e){
              ...
              if(dateStr>=wantStart && dateStr<=wantEnd){
                evRows.push({date:dateStr, title:e.getTitle(), time:timeStr, type:'event', calendar:cName});
              }
            });
          }catch(calErr){ /* this specific calendar isn't reachable right now (revoked share, typo, etc.) — skip it rather than fail the whole fetch */ }
        });
      }
      if(!calIdsToFetch.length || !calendarsUsed.length){
        // Nothing configured, or every configured calendar failed — fall
        // back to the default so events don't just silently disappear.
        var cal=CalendarApp.getDefaultCalendar();
        calendarsUsed.push(cal.getName());
        cal.getEvents(evStart,evEnd).forEach(function(e){
          var st=e.getStartTime();
          var dateStr=Utilities.formatDate(st,TZ,'yyyy-MM-dd');
          var timeStr=e.isAllDayEvent()?'All day':Utilities.formatDate(st,TZ,'h:mm a');
          if(dateStr>=wantStart && dateStr<=wantEnd){
            evRows.push({date:dateStr, title:e.getTitle(), time:timeStr, type:'event', calendar:cal.getName()});
          }
        });
      }

      // ── Also pull Birthdays (Google's auto-generated Contacts birthday calendar) ──
      // Two attempts, because the classic CalendarApp service is well known
      // to sometimes fail to see/read this particular system calendar even
      // when it's visible in the Calendar UI. Try the Advanced Calendar
      // Service (Calendar API v3) first, since it reaches calendars the
      // legacy service sometimes can't; fall back to the original
      // CalendarApp approach if the advanced service isn't enabled for
      // this project or also comes back empty. Whichever path actually
      // found something is reported back so this is diagnosable from the
      // System Check rather than a silent guess.
      var bdayRows=[];
      var bdaySource='none';
      try{
        if(typeof Calendar!=='undefined' && Calendar.Events){
          var bdayCalId='addressbook#contacts@group.v.calendar.google.com';
          var advResult=Calendar.Events.list(bdayCalId, {
            timeMin: evStart.toISOString(),
            timeMax: evEnd.toISOString(),
            singleEvents: true
          });
          if(advResult && advResult.items && advResult.items.length){
            bdayRows=advResult.items.map(function(ev){
              var dateStr=(ev.start && (ev.start.date || (ev.start.dateTime?ev.start.dateTime.slice(0,10):'')))||'';
              return {date:dateStr, title:ev.summary||'', time:'', type:'birthday'};
            }).filter(function(r){return r.date && r.date>=wantStart && r.date<=wantEnd;});
            if(bdayRows.length) bdaySource='advanced-api';
          }
        }
      }catch(advErr){ /* Advanced Calendar Service not enabled for this project, or this calendar ID not reachable this way — fall through to the legacy attempt below */ }
      ...
      if(!bdayRows.length){
        try{
          ...
          if(bdayCal){
            ...
            if(bdayRows.length) bdaySource='legacy-calendarapp';
          }
        }catch(be){ /* birthday calendar not accessible via the legacy service either — skip silently */ }
      }

      return out({events:evRows.concat(bdayRows), calendarsUsed:calendarsUsed, birthdaysFound:bdayRows.length, birthdaySource:bdaySource, scriptTimeZone:Session.getScriptTimeZone(), usedTimeZone:TZ});
    }catch(e){
  ...
  if(data.action==='diagnoseCalendars'){
    try{
      var allCals=CalendarApp.getAllCalendars();
      var selCalId=getSettingValue('selectedCalendarId','');
      var calList=allCals.map(function(c){
        ...
        return {name:c.getName(), id:c.getId(), isSelected:c.isSelected(), isOwned:c.isOwnedByMe(), isBirthdayCalendar:isBday, isChosenForEvents:(c.getId()===selCalId)};
      });
      var defCal=CalendarApp.getDefaultCalendar();
      var activeCal=selCalId ? (function(){ try{ return CalendarApp.getCalendarById(selCalId); }catch(e){ return null; } })() : null;
      return out({
        defaultCalendarId: defCal.getId(),
        defaultCalendarName: defCal.getName(),
        selectedCalendarId: selCalId,
        selectedCalendarName: activeCal ? activeCal.getName() : '',
        usingCalendarName: activeCal ? activeCal.getName() : defCal.getName(),
  ...
  function getParentDocsFolder(){
    var parentName='Deep Pocket Client Documents';
  ...
  // Read-only reachability test for the central feedback sheet — used by
  // both diagnostic reports. Deliberately never writes anything (a health
  // check silently seeding fake rows into Deep Pocket's real feedback
  // sheet would be its own kind of bug), just confirms this specific
  // script account can actually open it. Only checks app/product feedback
  // — retail client feedback is local to each firm's own sheet, not part
  // of this central one at all.
  if(data.action==='testCentralFeedbackSheet'){
    try{
      var tcfSs=SpreadsheetApp.openById(CENTRAL_FEEDBACK_SHEET_ID);
      var tcfName=tcfSs.getName();
      var tcfSh=tcfSs.getSheetByName('FEEDBACK');
      return out({success:true, sheetName:tcfName, feedbackTabExists:!!tcfSh, rowCount:tcfSh?Math.max(0,tcfSh.getLastRow()-1):0});
    }catch(e){
      return out({success:false, error:'Could not reach the central feedback sheet \u2014 it likely isn\'t shared with this account yet: '+e.message});
    }
  }

  // ── Product/app feedback — a FIRM (via the advisor using the CRM
  // itself) sending feedback about the APP to Deep Pocket, distinct from
  // submitFeedback above (a CLIENT giving feedback about their advisor).
  // Deliberately just 3 columns — Firm Name, Category, Feedback — kept
  // simple on purpose. Lands in the same central sheet as client
  // feedback, just a separate tab, so this firm's own script account
  // already has the access it needs (no second URL/endpoint to configure
  // per firm, which is exactly what caused this to fail unconditionally
  // before this fix).
  if(data.action==='submitProductFeedback'){
    try{
      var pfMessage=String(data.message||'').trim();
      if(!pfMessage) return out({success:false, error:'No message given'});
      var pfSs;
      try{ pfSs=SpreadsheetApp.openById(CENTRAL_FEEDBACK_SHEET_ID); }
      catch(pfOpenErr){ return out({success:false, error:'Could not reach the central feedback sheet — it may not be shared with this account yet.'}); }
      var pfSh=pfSs.getSheetByName('FEEDBACK')||pfSs.insertSheet('FEEDBACK');
      if(pfSh.getLastRow()===0){
        var pfHdr=[['Firm Name','Category','Feedback']];
        var pfHr=pfSh.getRange(1,1,1,3);
        pfHr.setValues(pfHdr);
        pfHr.setFontWeight('bold').setBackground('#C0392B').setFontColor('#FFFFFF');
        pfSh.setFrozenRows(1);
      }
      var pfFirmName=String(data.firmName||'').trim()||'(Firm name not set)';
      var pfCategory=String(data.category||'Other').trim();
      pfSh.appendRow([pfFirmName, pfCategory, pfMessage]);
      return out({success:true});
    }catch(e){
      return out({success:false, error:e.message});
    }
  ...
  if(data.action==='submitFeedback'){
    try{
      ...
      // Local to THIS firm's own sheet — not the central Deep Pocket sheet.
      // "Central" is reserved for feedback ABOUT THE APP from the firms
      // Deep Pocket sells this CRM to (see submitProductFeedback); this is
      // a retail client rating their own advisor, which is that firm's own
      // business data, not Deep Pocket's. Self-creates the FEEDBACK tab on
      // the firm's own sheet if it doesn't exist yet, same as it always did.
      ...
      // Referral handling — a referral is a real, actionable lead for this
      // firm, always local. Uses the REAL client name for "Referred By",
      // never "Anonymous", since a referral with no identifiable source is
      // useless to the advisor.
  ...
  // ── Practice Intelligence Centre — structured, evidence-based analysis
  // for one of the 14 fixed questions, rather than free-form chat. ──
  if(data.action==='getInsightQuestions'){
  ...
  // ── Daily AI Insights email — Settings-driven config (enable/disable,
  // recipient, delivery hour, timezone) instead of hardcoded values. ──
  if(data.action==='getDailyInsightsSettings'){
    try{
      return out({success:true,
        enabled: getSettingValue('dailyInsightsEnabled','')==='true',
        recipientEmail: getSettingValue('dailyInsightsRecipient',''),
        deliveryHour: parseInt(getSettingValue('dailyInsightsHour','8'),10)||8,
        timezone: getSettingValue('dailyInsightsTimezone','Asia/Kolkata'),
        scriptTimeZone: Session.getScriptTimeZone()
      });
    }catch(e){ return out({success:false, error:e.message}); }
  }
  if(data.action==='saveDailyInsightsSettings'){
    try{
      if(!verifyIsAdmin(data.requestingUserId)) return out({success:false, error:'Not authorized'});
      setSettingValue('dailyInsightsEnabled', data.enabled?'true':'false');
      if(data.recipientEmail!==undefined) setSettingValue('dailyInsightsRecipient', String(data.recipientEmail||'').trim());
      if(data.deliveryHour!==undefined) setSettingValue('dailyInsightsHour', String(parseInt(data.deliveryHour,10)||8));
      if(data.timezone!==undefined) setSettingValue('dailyInsightsTimezone', String(data.timezone||'Asia/Kolkata'));
      // Reflect the enable/disable + hour change into the actual trigger
      // immediately — the advisor shouldn't need a separate "apply" step.
      if(data.enabled) setupInsightsTrigger(); else removeInsightsTrigger();
      return out({success:true});
    }catch(e){ return out({success:false, error:e.message}); }
  }
  if(data.action==='sendTestInsightsEmail'){
    try{
      if(!verifyIsAdmin(data.requestingUserId)) return out({success:false, error:'Not authorized'});
      return out(sendDailyInsightsEmail(true)); // force=true — always sends, ignores enabled/dedup state
    }catch(e){ return out({success:false, error:e.message}); }
  }

  // ── Automatic Client Email → Task — Settings config + status. Saving
  // this does NOT change which inbox GmailApp actually reads (see the
  // comment on getClientEmailMonitoringStatus above) — it only records
  // what the admin expects, so a mismatch can be caught and shown plainly
  // instead of the feature silently doing nothing against the wrong inbox. ──
  if(data.action==='getClientEmailMonitoringStatus'){
    try{ return out({success:true, status:getClientEmailMonitoringStatus()}); }
    catch(e){ return out({success:false, error:e.message}); }
  }
  if(data.action==='saveClientEmailMonitoringInbox'){
    try{
      if(!verifyIsAdmin(data.requestingUserId)) return out({success:false, error:'Not authorized'});
      setSettingValue('clientEmailMonitoringInbox', String(data.inbox||'').trim());
      return out({success:true, status:getClientEmailMonitoringStatus()});
    }catch(e){ return out({success:false, error:e.message}); }
  }
...
// ═══════════════════════════════════════════
const BACKUP_FOLDER_NAME='Deep Pocket CRM Backups';
